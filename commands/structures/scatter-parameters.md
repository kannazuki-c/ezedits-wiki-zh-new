...
### 遮罩过滤器: <mark style="color:orange;">`-m <mask>`</mark>

启用过滤不匹配由 `-m <mask>` 给定的遮罩的放置位置。放置位置必须满足遮罩才能放置结构。
...
密度值是一个百分比。它确定在多少百分比的表面方块上放置结构。具体来说，它确定在方向过滤器和遮罩过滤器应用 _之后_ 的位置的百分比。
...
例如，如果你使用掩码过滤器将放置限制在特定方块上，而该方块在你的选区中很少出现，例如使用以下区域和 `-m sea_lantern`，
...
### 掩码覆盖方块: <mark style="color:orange;">`-l <pattern>`</mark>

放置所有结构后，将区域内所有与掩码过滤器 (`-m`) 匹配的未受影响的表面方块替换为给定的方块。(`-m` 必须设置才能使此标志生效。)
...
>
> 运行 ezbrush scatter **使用** `-b pink`，其中粉红色羊毛在本例中只是某个随机方块，会覆盖受影响的区域，结合 `-m clay` 掩码过滤器，后续的笔刷点击不会在那里放置任何新形状，即使区域重叠。
>
