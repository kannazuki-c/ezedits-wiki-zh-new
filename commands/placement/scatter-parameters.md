...
[`//ezscatter`](./#overview) 和 [`//ezbrush scatter`](./#overview) 在一个区域内放置多个结构。这些命令选择的位置可以使用本页面描述的参数进行自定义。

`scatter` 首先从由 `-h <region>` 定义的区域中提取所有表面方块（所有接触空气的非空气方块）。您可以使用 `-d <directions>` & `-e <threshold>` 和 `-m <mask>` 过滤出不符合特定条件的表面。在剩余的表面上，根据给定的密度 `-n <density>` 和均匀性 `-u <iterations>` 计算放置位置分布，然后放置结构。
...
* 默认值：
  * 对于 `//ezscatter` 为您当前选定的区域
    * <mark style="color:blue;">`-h Active(UseOriginalPosition:true)`</mark>
  * 或对于 `//ezbrush scatter` 为点击位置周围的 40x40x40 立方体区域。
...
* <mark style="color:orange;">**`Active`**</mark> (<mark style="color:orange;">**`A`**</mark>): 你当前选中的区域。
  * 与前三个区域选项不同，这最后两个选项在世界中有固有的位置。你可以选择通过将其移动到玩家位置（`//ezsc`）或点击位置（`//ezbr sc`）来覆盖其位置，或使用 <mark style="color:blue;">**`UseOriginalPosition`**</mark> (<mark style="color:blue;">**`P`**</mark>) 参数使用其固有位置。\
    `//ezsc` 的区域参数的默认值实际上是 `-h Active(UseOriginalPosition:true)`。如果你将其设置为 false，则命令将以区域移动到你的玩家位置的方式执行。
* 默认情况下，区域的中心位于目标位置。你可以使用 <mark style="color:blue;">**`Offset`**</mark> (<mark style="color:blue;">**`O`**</mark>) 参数定义偏移，以相对于其目标位置移动它。默认值为 `(0,0,0)`。
...
选区使用粉红色羊毛进行可视化以便清晰显示（使用 [-l flag](scatter-parameters.md#mask-cover-pattern-l)）。
...
启用过滤掉朝向特定方向的表面上的放置位置。

`-d <directions>` 参数定义了基本方向列表（上、下、北、东、南、西），放置位置所在表面必须朝向这些方向。如果放置位置不满足此条件，则不会在该处放置结构。
...
**`-d west,up`** **`-e 0.5`**（散布点必须在朝向西方或向上的表面上。）

<img src="../../.gitbook/assets/ScatterDirectionalFilter_example.png" alt="" data-size="original">

**`-d up`** **`-e <threshold>`**（散布点必须在朝向大致向上的表面上）

* 从 **`-e -1.0`** 开始（最弱的过滤阈值，放置所有形状）
* 在 **`-e 0.0`** 处暂停（过滤掉一半的所有方向）
* 在 **`-e 1.0`** 处结束（最强的过滤阈值，不再放置任何形状）。
...
启用过滤掉不匹配 `-m <mask>` 给定蒙版的放置位置。放置位置必须满足蒙版才能放置结构。

<details>

<summary><mark style="color:blue;">示例</mark></summary>

`//ezsc Clipboard S C`` `**`-m <mask>`**（剪贴板为默认的原版橡木树）

使用以下蒙版：

* **`-m red`**（仅选择红色羊毛方块上的放置位置）
* **`-m !red`**（仅选择红色羊毛方块以外任何方块上的放置位置）
* **`-m =y>95`**（仅选择 y>95 以上的放置位置）
...
**`//ezsc Clipboard C C -n <density>`**（剪贴板为默认的原版橡木树）
...
对于这类情况，如果你想在特定方块的每个实例上放置结构，应该使用 `-n 100%`。在我们的示例中执行 `//ezsc Clipboard -m sea_lantern -n 100%` 会得到：
...
### 均匀性: <mark style="color:orange;">`-u <iterations>`</mark> <a href="#uniformity-u" id="uniformity-u"></a>

确定所有放置位置的均匀分布程度。需要一个正整数，包括 0。

默认为 `15`。

均匀性算法通过从完全随机的放置位置开始，然后迭代地将所有位置相互推开来工作。此参数设置要执行的推开迭代次数。因此，0 表示区域内的放置位置完全随机。

<details>

<summary><mark style="color:blue;">示例</mark></summary>

`//ezsc Clipboard C C`` `**`-u <iterations>`** (剪贴板为默认原版橡木树)
...
放置所有结构后，将区域内与[蒙版过滤器 (`-m`)](scatter-parameters.md#mask-filter-m) 匹配的所有未受影响的表面方块替换为给定的方块。(`-m` 必须设置才能使此标志生效。)

这是一个小众实用选项，用于在相邻区域多次应用散布但不想在已经散布过的区域放置结构的情况。因此，使用此标志，您可以（在工作流程中临时）用给定的方块覆盖区域内的所有表面方块，使得任何与已覆盖区域重叠的后续散布操作都不会放置结构，因为表面方块已被"覆盖"。
...
运行 ezbrush scatter **使用** `-l pink`，其中粉红色羊毛在本例中只是某个随机方块，覆盖受影响的区域，使得结合 `-m clay` 蒙版过滤器，后续笔刷点击不会在那里放置任何新形状，即使区域重叠。
...
### 修剪选区外: <mark style="color:orange;">`-t`</mark> <a href="#trim-outside-selection-t" id="trim-outside-selection-t"></a>

默认情况下，`scatter` 将在当前选区内确定放置位置，但如果放置位置位于当前选区的边界，将在选区外放置方块。您可以使用此 `-t` 标志来切除任何此类方块（防止它们被放置）。
...
那么执行不带该标志的 ezsc 命令将导致方块可能被放置在选区外。只有放置/原点位置被限制在选区内。
